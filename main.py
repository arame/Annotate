...
def main():
    ...
    sent = Sentiment()
    i = 0
    Helper.printline(f"Iteraate through {len(list_dirs)} countries")
    for country in list_dirs:
        ...
        csv_input = csv_input.drop(csv_input.query(f'sentiment=={sent.NEUTRAL}'))
        Helper.printline(f"{i}. Country: {country} saving {len(csv_input)} entries")
